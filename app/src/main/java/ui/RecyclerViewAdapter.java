...
public class RecyclerViewAdapter extends RecyclerView.Adapter<RecyclerViewAdapter.ViewHolder> {
    ...
    public class ViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public void onClick(View view) {
            ...
            Item item=itemList.get(position);
            switch (view.getId()){
                case R.id.editButton:  //Modifica Esercizio

                    editItem(item);
                    break;

                case R.id.deleteButton:  //Elimina Esercizio

                    deleteItem(item.getId());
                    break;

                case R.id.timerButton:   //Timer
                    if (!item.getItemRest().toString().isEmpty()) {
                        Intent intent = new Intent(view.getContext(), ActivityTimer.class);
                        intent.putExtra("Time", item.getItemRest().toString().trim());  //Passo i secondi inseriti all'attività del timer
